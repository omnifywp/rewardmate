...
function rewardmate_render_admin_page_header($title, $description = '', $action_url = '', $action_label = '') {
    $current_page = isset($_GET['page']) ? sanitize_key(wp_unslash($_GET['page'])) : 'rewardmate';

    $pro_badge = '';
    if (function_exists('rewardmate_has_pro_addon') && rewardmate_has_pro_addon()) {
        $pro_badge = ' <span class="rewardmate-pro-badge" style="background-color:#c88719; color:#fff; font-size:0.75em; padding:2px 6px; border-radius:4px; font-weight:bold; margin-left:5px; vertical-align:middle; text-transform:uppercase;">' . esc_html__('Pro', 'rewardmate') . '</span>';
    }
    ...
    echo '<span>' . esc_html__('Omnify Customer Rewards', 'rewardmate') . $pro_badge . '</span>';
    ...
    echo '</div>';
    echo '<nav class="rewardmate-admin-tabs" aria-label="' . esc_attr__('Omnify Customer Rewards admin navigation', 'rewardmate') . '">';
...
function rewardmate_register_admin_menu() {
    ...
    add_menu_page(
        esc_html__('RewardMate', 'rewardmate'),
        esc_html__('RewardMate', 'rewardmate'),
        'manage_woocommerce',
        'rewardmate',
    ...
    add_submenu_page(
        'rewardmate',
        esc_html__('RewardMate Dashboard', 'rewardmate'),
        esc_html__('Dashboard', 'rewardmate'),
        'manage_woocommerce',
        'rewardmate',
        'rewardmate_render_dashboard_page'
    );
...
function rewardmate_render_free_dashboard_page() {
    $links = [
        [
            'title' => __('Settings', 'rewardmate'),
            'desc'  => __('Configure core point values, earn ratios, max redemptions, and email templates.', 'rewardmate'),
            'url'   => admin_url('admin.php?page=rewardmate-settings'),
        ],
        [
            'title' => __('Adjust Points', 'rewardmate'),
            'desc'  => __('Add or deduct customer points manually and gift bonus points to users.', 'rewardmate'),
            'url'   => admin_url('admin.php?page=rewardmate-adjust-user-points'),
        ],
        [
            'title' => __('Points History', 'rewardmate'),
            'desc'  => __('Review customer-level point changes across check-ins, purchases, and manual updates.', 'rewardmate'),
            'url'   => admin_url('admin.php?page=rewardmate-points-history'),
        ],
    ];
    ...
    rewardmate_render_admin_page_header(
        __('Dashboard', 'rewardmate'),
        __('Manage your WooCommerce loyalty program from one clean workspace.', 'rewardmate'),
        admin_url('admin.php?page=rewardmate-settings'),
        __('Open Settings', 'rewardmate')
    );

    // Welcome Box / Core stats
    echo '<div class="rewardmate-admin-card rewardmate-admin-dashboard" style="margin-bottom:20px;">';
    echo '<h2>' . esc_html__('Welcome to Omnify Customer Rewards!', 'rewardmate') . '</h2>';
    echo '<p>' . esc_html__('Your loyalty program is active. Customers can earn points on purchases, perform daily check-ins, and redeem points for discounts during checkout.', 'rewardmate') . '</p>';
    echo '</div>';

    // Quick Actions grid
    echo '<div class="rewardmate-admin-card rewardmate-admin-dashboard">';
    echo '<h2>' . esc_html__('Quick Actions', 'rewardmate') . '</h2>';
    ...
    echo '<h2 style="color:#c88719;">' . esc_html__('Upgrade to Omnify Customer Rewards WooCommerce Pro', 'rewardmate') . '</h2>';
    echo '<p>' . esc_html__('Unlock advanced features to maximize your shop\'s conversion rate and lifetime value:', 'rewardmate') . '</p>';
    echo '<ul style="list-style-type:disc; margin-left:20px; margin-bottom:20px;">';
    echo '<li><strong>' . esc_html__('Referral System', 'rewardmate') . '</strong>: ' . esc_html__('Acquire new customers with dual-sided viral referral rewards.', 'rewardmate') . '</li>';
    echo '<li><strong>' . esc_html__('Product & Category Multipliers', 'rewardmate') . '</strong>: ' . esc_html__('Set rules like 2x points on select items or exclude discounted category sales.', 'rewardmate') . '</li>';
    echo '<li><strong>' . esc_html__('Abuse Protection & Fraud Filters', 'rewardmate') . '</strong>: ' . esc_html__('Prevent points farming, shared IP registrations, and check-in abuse.', 'rewardmate') . '</li>';
    echo '<li><strong>' . esc_html__('REST API & Webhooks', 'rewardmate') . '</strong>: ' . esc_html__('Connect to mobile apps, physical POS systems, and Klaviyo/ERP platforms.', 'rewardmate') . '</li>';
    echo '<li><strong>' . esc_html__('Analytics Dashboard & Point Ledger', 'rewardmate') . '</strong>: ' . esc_html__('Detailed order audit trails, charts, and liability reporting.', 'rewardmate') . '</li>';
    echo '</ul>';
    echo '<a class="button button-primary wp-element-button" style="background-color:#c88719; border-color:#c88719;" href="https://omnifywp.com/rewardmate-pro" target="_blank">' . esc_html__('Learn More About Pro', 'rewardmate') . '</a>';
    echo '</div>';
